...
public class ECSManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < numAsteroids; i++)
        {
            ...
            var s = new float3(UnityEngine.Random.Range(5,15), UnityEngine.Random.Range(5, 10), UnityEngine.Random.Range(5, 15));
            s *= UnityEngine.Random.Range(1, 10);
            manager.AddComponent(instance, ComponentType.ReadWrite<NonUniformScale>());
            manager.SetComponentData(instance, new NonUniformScale { Value = s });
